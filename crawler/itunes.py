...
class ItunesSpider(scrapy.Spider):
    ...
    def parse(self,response):
        ...
        app_website = response.xpath()
        app_website = app_website.replace('\n', '').replace(' ', '')
        print(app_website)

        app_support_website = response.xpath()
        app_support_website = app_support_website.replace(
            '\n', '').replace(' ', '')
        print(app_support_website)

        app_purchases_text = response.xpath()
        app_purchases_text = app_purchases_text.replace('\n', '').replace(' ', '')
        print(app_purchases_text)
